...
Real-time data is pretty simple acually. The data is stored as JSON and in a noSQL cloud database and the data is synced across all client in realtime. It even remains available if your app goes offline. 
...
The next part is very similar to the simple way to setup fetch with firebase that i display in [this article](https://calo.dev/blog/firebase-database-fetch). However, there is some changes:
...
First of we set our query with the db and the `c` in this context is the `collection name` we want to fetch, just to make it a little bit more dynamic.
...
This mean that we are listening to the event and there after we need to do some cleanup when this component thats using it unmount with:
...
And now for using the hook, we just reuse the same code we had before in our simple fetching verison. You just need to import it and declare which collection you want then everytime you add, remove or change a document it will update.
