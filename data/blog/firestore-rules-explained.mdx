...
When you've done this you would like to click the `Rules tab` in the top left of the page (underneith the Cloud Firestore title). Then this view will pop up as shown in the image below.

![Firestore rules page](/static/images/firestorerulespage.webp)

All these lines may be confusing at first but not to worry, they are accually pretty easy to understand!
...
This line is always present in your rules but the current path just takes `any` request to your database. The `{}` curlybraces represent a `wildcard` in this context, wildcard meaning `any` database and you may have seen the star `*` being used the same way. 
